...
public class ArmorGlobalItem : GlobalItem
{
    ...
    public int SentrySlots { get; set; }
    ...
    public override void UpdateEquip(Item item, Player player) {
        ...
        player.endurance += DamageReduction / 100f; // Need to do it this way as I assumed DR was an int, it was not an int...
    ...
    public override void ModifyTooltips(Item item, List<TooltipLine> tooltips) {
        ...
        if (FlightTime > 0) {
            float flightTimeInSeconds = FlightTime / 60f;
    ...
    public override GlobalItem Clone(Item from, Item to) {
        ArmorGlobalItem clone = (ArmorGlobalItem)base.Clone(from, to);
        clone.MaxHP = MaxHP;
...
public class ArmorPlayer : ModPlayer
{
    ...
    public override void Load() => On_Player.GetWingStats += Player_GetWingStats;

    public override void Unload() => On_Player.GetWingStats -= Player_GetWingStats;

    private WingStats Player_GetWingStats(Terraria.On_Player.orig_GetWingStats orig, Player self, int wingID) {
        WingStats ret = orig(self, wingID);
        ArmorPlayer modPlayer = self.GetModPlayer<ArmorPlayer>();
    ...
    public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers) => modifiers.CritDamage += CritDamage;
}